...
class ShapeApp : public QMainWindow
{
  Q_OBJECT
...
/// Slider declarations ---- y -----
 ...
 private:
  // Designer form
  Ui_ShapeApp* ui;
  bool _cube_axis_actor1_on;
  bool _scalar_bar_vis;
  double _minH;
  double _maxH;
  vtkRenderer* _ren1;
  vtkCellArray*_polys;
  ...
  vtkScalarBarActor* _scalarBar;
  vtkLookupTable *_lut;
  shp_surface *_s;
  bool _check_self_intersection;
  bool _axis_on;
  double _cscale, _scale_fac, _bgrdr, _bgrdg, _bgrdb;		// scale factor for clks necessary when importing shapes and default background color
